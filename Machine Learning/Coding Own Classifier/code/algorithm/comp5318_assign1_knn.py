...
#runs the KNN classifier.  Includes reading of data, cleaning, data munging/parsing,
#cross validation, classifier evaluation, score/metrics 
def runKNN(path_data, trainingDataFile, trainingLabelData, testingDataFile, crossvalidation, kfold):
    path_data = os.path.join(path_data, "input") 
    
    if (crossvalidation): print("There is no cross validation for KNN.")
